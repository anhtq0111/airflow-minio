...
@dag(dag_id="etlpipeline__exact101",
    ...
    description='A DAG writed by Quoc Anh to run dbt models for exact101 in the staging',
...
def etlpipeline__exact101():
    ...
    # Define bash command for dbt run operational_metadata for all sources
    bash_command_metadata = f"""dbt run --profiles-dir {PROFILES_DIR} --project-dir {PROJECT_DIR}  -s path:{METADATA_PATH}/*"""
    ...
    start() >> run_operational_metadata >> run_staging >> run_rawvault >> end()
